...
test/preview at [www.mysitecoreproject.localhost](https://www.mysitecoreproject.localhost).
Be sure to watch the `rendering` container logs for compilation errors. To debug, you
can attach to the `MySitecoreProject.exe` process inside the `rendering` container.
